...
assistant.order( 'kkbox/kkboxApi_kmc', function ( self ) {
    assistant.log.setMsg( {
        KKBOx_fetchHTTPStatusCode: 'The "%s" HTTP status code of UrlFetchApp.',
    } );
     ...
     * @throws {Error} Error(21, kkbox_fetchHTTPStatusCode)
     * @return {Array} 新增的收集清單。
     */
    function kkboxApi_kmc(
        arrKkboxCollectIdList,
        strKmcCategory, numOffsetDate, numLimitCount
    ) {
        var len, idx, val;
        var dataChartDaily, dataSong, dataSong_content;
        var newCollectList = [];

        dataChartDaily = fhrData_chartDaily( strKmcCategory, numOffsetDate, numLimitCount );
     ...
     * 音樂排行榜曲風分類清單。
     *
     * @memberof module:kkbox.kkboxApi_kmc.
     * @var {Array} kmcCategoryList
     */
    kkboxApi_kmc.kmcCategoryList = Object.keys( kmcCategoryInfo );

    function fhrData_chartDaily( strKmcCategory, numOffsetDate, numLimitCount ) {
        var fhr, songList;
        var dataChartDaily;

        fhr = fhrKkboxData(
            '排行榜資料',
            getChartDailyUrl( strKmcCategory, numOffsetDate, numLimitCount )
        );
     ...
     * @return {String}
     */
    function getChartDailyUrl( strKmcCategory, numOffsetDate, numLimitCount ) {
