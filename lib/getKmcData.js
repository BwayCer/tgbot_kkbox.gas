...
assistant.order( 'kkbox/getKmcData', function ( self ) {
     ...
     * 取得排行榜資料。
     *
     * @memberof module:kkbox.getKmcData.
     * @func run
     */
    getKmcData.run =  function getKmcData() {
        var lenA, idxA;
        var kkboxIdList, weekUpdateList, todayKmcTargetList;
        var kmcCategory, kmcNewCollectList;
        var chatId, photoId, audioId;
        var numOffsetDate = -1;

        var dbSheet_keyVal = assistant.gasdb( 'kkbox', 'keyVal' );
        ...
        todayKmcTargetList = weekUpdateList[
            getDateOffset( numOffsetDate ).getDay()
        ];
        ...
        for ( idxA = 0, lenA = todayKmcTargetList.length; idxA < lenA ; idxA++ ) {
            ...
            tgPush.pushNewAudioAll( chatId, kkboxIdList, kmcNewCollectList );
        }
    };
    ...
    function fhrData_chartDaily( strKmcCategory, numOffsetDate ) {
        ...
        dataChartDaily = fhr.content.data.charts.song.reduce(
            function ( arrAccumlator, val ) {
                if ( val.is_auth ) {
                    arrAccumlator.push( {
                        type:          val.type,
                        id:            val.song_id,
    ...
    function getChartDailyUrl( strKmcCategory, numOffsetDate ) {
        return 'https://kma.kkbox.com/charts/api/v1/daily'
            ...
            + '&date=' + getDateFormat( +getDateOffset( numOffsetDate ), 'date' )
            + '&lang=tc'
    ...
    function getDateOffset( numOffsetDate ) {
